...
IF (NOT DEFINED ESCARGOT_BUILD_64BIT_FORCE_LARGE)
    IF (ESCARGOT_HOST STREQUAL "linux" OR ESCARGOT_HOST STREQUAL "darwin" OR ESCARGOT_HOST STREQUAL "android")
        SET (ESCARGOT_BUILD_64BIT_FORCE_LARGE OFF)
    ELSE()
...
# Compressed values require a reserved cage-base register in the interpreter.
# Probe before setting the engine and GCutil definitions so an unsupported
# compiler selects full-width pointers for both targets.
IF (ESCARGOT_BUILD_64BIT AND NOT ESCARGOT_BUILD_64BIT_LARGE AND
    NOT ESCARGOT_BUILD_64BIT_FORCE_LARGE AND
    (ESCARGOT_ARCH STREQUAL "x64" OR ESCARGOT_ARCH STREQUAL "x86_64" OR
     ESCARGOT_ARCH STREQUAL "aarch64" OR ESCARGOT_ARCH STREQUAL "arm64"))
    IF (ESCARGOT_ARCH STREQUAL "x64" OR ESCARGOT_ARCH STREQUAL "x86_64")
        SET (ESCARGOT_CAGE_REGISTER_FLAG "-ffixed-r15")
    ELSE()
        SET (ESCARGOT_CAGE_REGISTER_FLAG "-ffixed-x27")
    ENDIF()
    INCLUDE (CheckCXXCompilerFlag)
    CHECK_CXX_COMPILER_FLAG ("${ESCARGOT_CAGE_REGISTER_FLAG}" ESCARGOT_HAS_CAGE_REGISTER_FLAG)
    IF (NOT ESCARGOT_HAS_CAGE_REGISTER_FLAG)
        MESSAGE (WARNING "${ESCARGOT_CAGE_REGISTER_FLAG} is unavailable; using full 64-bit pointers")
        SET (ESCARGOT_BUILD_64BIT_LARGE ON)
    ENDIF()
ENDIF()
...
OPTION (ESCARGOT_FORCE_TLS_DIALECT_GNU2 "Force enable gnu2 tls dialect" ON)

INCLUDE (CheckCXXCompilerFlag)
INCLUDE (CheckCCompilerFlag)

IF (ESCARGOT_FORCE_TLS_DIALECT_GNU2 AND NOT ESCARGOT_TLS_ACCESS_BY_ADDRESS AND NOT ESCARGOT_TLS_ACCESS_BY_PTHREAD_KEY)
    CHECK_CXX_COMPILER_FLAG ("-mtls-dialect=gnu2" HAS_TLS_DIALECT_GNU2)
    IF (HAS_TLS_DIALECT_GNU2)
        MESSAGE (STATUS "Enabling -mtls-dialect=gnu2 for ESCARGOT_CXXFLAGS")
        SET (ESCARGOT_CXXFLAGS ${ESCARGOT_CXXFLAGS} -mtls-dialect=gnu2)
    ENDIF ()

    CHECK_C_COMPILER_FLAG ("-mtls-dialect=gnu2" HAS_C_TLS_DIALECT_GNU2)
    IF (HAS_C_TLS_DIALECT_GNU2)
        MESSAGE (STATUS "Enabling -mtls-dialect=gnu2 for ESCARGOT_THIRDPARTY_CFLAGS")
        SET (ESCARGOT_THIRDPARTY_CFLAGS ${ESCARGOT_THIRDPARTY_CFLAGS} -mtls-dialect=gnu2)
    ENDIF ()
ENDIF ()
