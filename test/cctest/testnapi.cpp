...
TEST(Napi, TwoFunctionArguments)
{
    ...
    // Intentionally NOT dlclose(handle): a wrapped object created by this addon
    // can outlive the test on the GC heap and have its finalizer - whose code
    // lives inside this .so - invoked later (e.g. by a subsequent test's GC).
    // Unloading the .so would turn that into a call into unmapped memory
    // (SIGSEGV). Addon handles are process-lifetime in this test binary.
}
...
// Boehm's conservative scan stops mistaking a stale MyObject* left in a
// callee-saved register / spilled stack slot by a previous Evaluator::execute
// call for a live root. Recurses to reach deeper than the frames those calls
// used. `volatile` + the sink check keep the compiler from eliding it.
