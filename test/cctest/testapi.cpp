...
int main(int argc, char* argv[])
{
    ...
    CHECK("globalObject() is not null", !globalObject->isNull(es));
    CHECK("globalObject() is object", globalObject->isObject(es));
    Escargot::ValueRef* jsmath
        = globalObject->getProperty(es, Escargot::StringRef::fromASCII("Math"));
    CHECK("global.Math is object", jsmath->isObject(es));
    Escargot::ValueRef* jspi
        = ((Escargot::ObjectRef*)jsmath)->getProperty(
                es, Escargot::StringRef::fromASCII("PI"));
    printf("Math.PI = %f\n", jspi->toNumber(es));
