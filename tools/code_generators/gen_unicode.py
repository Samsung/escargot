...
def create_basic_plane_table():
    ...
    for continue_key in BASIC_PLANE_CONTINUE.keys():
        if not MERGED_ID_START.__contains__(continue_key):
    ...
    long_length_index = 0
    for key, value in MERGED_ID_START.items():
        if value >= 200:
                ...
                if (paragraph_title == ID_START_TITLE) or (paragraph_title == ID_CONTINUE_TITLE):
                    ...
                    while (unicode_range_line != "\n"):
                        ...
                        if (128 < start) and (start <= 65535):
                            if skip == 8:
                                if not BASIC_PLANE_START.__contains__(start):
                                    BASIC_PLANE_START[start] = length
                            else:
                                if not BASIC_PLANE_CONTINUE.__contains__(start):
...
def main():
    ...
    parser = ArgumentParser(description='Automated Unicode Identifier Table Generator')
    parser.add_argument('--derived_core_properties', metavar='FILE', action='store', required=True, help='specify the unicode data file')
    parser.add_argument('--dst', metavar='FILE', action='store', required=True, help='specify dst')
    ...
    ifdef_namespace ="#include \"parser/UnicodeIdentifierTables.h\"\n\nnamespace Escargot {\nnamespace EscargotLexer {\n"
    footer_namespace = "const uint16_t basicPlaneLength = sizeof(identRangeStart);\nconst uint16_t supplementaryPlaneLength = sizeof(identRangeStartSupplementaryPlane);\n} // namespace EscargotLexer\n} // namespace Escargot"
    ...
    generated_data.generate(args.dst)
    print("\n%sTables generated into %s %s\n" % (TERM_GREEN,args.dst,TERM_EMPTY))
