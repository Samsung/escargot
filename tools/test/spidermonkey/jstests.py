...
# NOTE(Escargot): A few modules under lib/ (e.g. lib/results.py) import their
# lib-internal sibling modules (progressbar, structuredlog, ...) as top-level
# module names rather than via the "lib" package. That relied on Python 2's
# implicit relative imports, which Python 3 no longer supports. Rather than
# editing those vendored files, put lib/ itself on sys.path so those imports
# keep resolving as before.
sys.path.insert(0, os.path.join(dirname(abspath(__file__)), 'lib'))
