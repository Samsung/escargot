...
// original source comes from https://gist.github.com/calebh/fd00632d9c616d4b0c14e7c2865f3085
...
struct static_max<arg1, arg2, others...> {
    static const unsigned int value = arg1 >= arg2 ? static_max<arg1, others...>::value : static_max<arg2, others...>::value;
};
...
public:
    ...
    variant(const variant<Ts...>& from)
        : variant_id(from.variant_id)
    {
        if (from.variant_id) {
            helper_t::copy(from.variant_id.value(), &from.data, &data);
        }
    }

    variant(variant<Ts...>&& from)
        : variant_id(from.variant_id)
    {
        if (from.variant_id) {
            helper_t::move(from.variant_id.value(), &from.data, &data);
        }
    }
    ...
    {
        if (variant_id) {
            helper_t::destroy(variant_id.value(), &data);
        }
...
template <typename... Ts>
using Variant = VariantImpl::variant<Ts...>;
