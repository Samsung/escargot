...
ASTBuffer::~ASTBuffer()
{
    ...
    for (size_t i = 0; i < m_astGCNodes.size(); i++) {
        m_astGCNodes[i]->~Node();
        GC_FREE(m_astGCNodes[i]);
    }
    m_astGCNodes.clear();
...
void* ASTBuffer::allocate(size_t size)
{
    ASSERT(size > 0);
...
void* ASTBuffer::allocateDestructibleNode(size_t size)
{
...
void* ASTBuffer::allocateGCNode(size_t size)
{
    void* ptr = GC_MALLOC_UNCOLLECTABLE(size);
    m_astGCNodes.push_back(static_cast<Node*>(ptr));
    return ptr;
}

void ASTBuffer::allocatePool()
{
    ASSERT(m_astPoolMemory != nullptr && m_astPoolEnd != nullptr);
    ASSERT(static_cast<size_t>(m_astPoolEnd - m_astPoolMemory) >= 0);
