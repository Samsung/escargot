...
public:
    ...
    virtual void generateExpressionByteCode(ByteCodeBlock* codeBlock, ByteCodeGenerateContext* context, ByteCodeRegisterIndex dstRegister) override
    {
        ...
        if (dstRegister == REGISTER_LIMIT) {
            codeBlock->pushCode(JumpIfEqual(ByteCodeLOC(m_loc.index), src0, src1, true, true), context, this->m_loc.index);
        } else {
            codeBlock->pushCode(BinaryStrictEqual(ByteCodeLOC(m_loc.index), src0, src1, dstRegister), context, this->m_loc.index);
        }
