...
-DESCARGOT_THREADING=ON -DCMAKE_BUILD_TYPE=Release -DESCARGOT_SHELL=ON -DESCARGOT_TEST=ON -DESCARGOT_TEMPORAL=ON -DESCARGOT_TLS_ACCESS_BY_ADDRESS=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DESCARGOT_EXPORT_ALL=ON -G Ninja
%else
cmake CMakeLists.txt -H./ -Bbuild/out_tizen_%{rpm} -DLIBDIR=%{_libdir} -DINCLUDEDIR=%{_includedir} -DTIZEN_MAJOR_VERSION='%{tizen_version_major}' \
-DESCARGOT_ARCH='%{tizen_arch}' -DESCARGOT_WASM='%{enable_wasm}' -DESCARGOT_DEBUGGER='%{enable_debugger}' -DESCARGOT_SMALL_CONFIG='%{enable_small_config}' \
-DESCARGOT_THREADING=ON -DCMAKE_BUILD_TYPE=Release -DESCARGOT_SHELL=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_POSITION_INDEPENDENT_CODE=ON -DESCARGOT_EXPORT_ALL=ON -G Ninja
%endif
